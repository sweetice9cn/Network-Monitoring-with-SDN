...
class ControllerOne(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(ControllerOne, self).__init__(*args, **kwargs)
        self.mac_to_port = {}
        self.arp_table = {}
    ...
    def switch_features_handler(self, ev):
        ...
        # set pre-installed rules for switches since controller one uses fixed flow paths
        if datapath.id == 1:
            self.add_switch1_rules(datapath, 40000)
        if datapath.id == 3:
            self.add_switch3_rules(datapath, 40000)
        if datapath.id == 4:
            self.add_switch4_rules(datapath, 40000)
        if datapath.id == 5:
            self.add_switch4_rules(datapath, 40000)

    def add_switch1_rules(self, datapath, priority = 1):
        self.add_UDP_rules(datapath, priority, "10.0.0.1", "10.0.0.2", 2)
        self.add_UDP_rules(datapath, priority, "10.0.0.2", "10.0.0.3", 3)

    def add_switch3_rules(self, datapath, priority = 1):
        self.add_UDP_rules(datapath, priority, "10.0.0.1", "10.0.0.2", 2)
        self.add_UDP_rules(datapath, priority, "10.0.0.1", "10.0.0.3", 2)

    def add_switch4_rules(self, datapath, priority = 1):
        self.add_UDP_rules(datapath, priority, "10.0.0.1", "10.0.0.2", 1)
        self.add_UDP_rules(datapath, priority, "10.0.0.2", "10.0.0.3", 2)

    def add_switch5_rules(self, datapath, priority = 1):
        self.add_UDP_rules(datapath, priority, "10.0.0.2", "10.0.0.3", 1)
        
    def add_UDP_rules(self, datapath, priority = 1, ipv4_src = None, ipv4_dst = None, fwd_port = None):
        parser = datapath.ofproto_parser
